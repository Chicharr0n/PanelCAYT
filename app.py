...
with st.sidebar:
    ...
    st.caption(f"DB: {st.session_state.get('db_connection_type', 'N/A')}")
            ...
            with tab_ficha:
                with st.form(key=f"form_ficha_{exp_numero}"):
                    ...
                    if st.form_submit_button("Guardar Ficha"):
                        db_manager.update_ficha_expediente(exp_numero, {'juzgado_nombre': selected_juzgado_nombre, 'secretaria_nombre': selected_secretaria_nombre, 'medida_cautelar_status': st.session_state[f"mc_{exp_numero}"], 'observaciones': st.session_state[f"obs_{exp_numero}"]})
