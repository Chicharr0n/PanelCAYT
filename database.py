...
def get_engine():
    ...
    try:
        ...
        # Conexión a Turso (libSQL remoto con autenticación por token)
...
class DatabaseManager:
    ...
    def sync_expedientes(self, df):
        """Sincroniza expedientes desde un DataFrame."""
    ...
    def get_all_data(self):
        """Devuelve DataFrames de todas las tablas principales."""
        with self.engine.connect() as conn:
        ...
        if not tareas.empty:
            tareas['fecha_vencimiento'] = pd.to_datetime(
                tareas['fecha_vencimiento'], errors='coerce'
            ).dt.date
            if 'completada' in tareas.columns:
                tareas['completada'] = tareas['completada'].fillna(False).astype(bool)

        if not notas.empty:
            notas['fecha_creacion'] = pd.to_datetime(
                notas['fecha_creacion'], errors='coerce'
            )

        if not movimientos.empty:
            movimientos['fecha'] = pd.to_datetime(
                movimientos['fecha'], errors='coerce'
            ).dt.date

        return expedientes, tareas, notas, movimientos

    def update_ficha_expediente(self, numero, data):
        """Actualiza campos de un expediente específico."""
        with self.engine.connect() as conn:
            stmt = db.text("""
                UPDATE expedientes
                SET juzgado_nombre=:jn, secretaria_nombre=:sn,
                    medida_cautelar_status=:mcs, observaciones=:o
                WHERE numero=:n
            """)
            conn.execute(stmt, {
                "jn": data['juzgado_nombre'],
                "sn": data['secretaria_nombre'],
                "mcs": data['medida_cautelar_status'],
    ...
    def add_item(self, table_name, data):
        """Agrega un registro genérico a la tabla indicada."""
        with self.engine.connect() as conn:
...
# ⚡ Instancia global lista para importar en otros módulos
_engine = get_engine()
init_db(_engine)
